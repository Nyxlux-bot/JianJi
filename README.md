...
-  **离线卷宗库与迁移**
  ...
  - **恢复预览与选择导入**：导入前会显示记录预览、重复 ID 数量、可勾选导入项，并支持“跳过重复/覆盖重复”冲突策略，导入后显示 `inserted/updated/skipped` 统计。

-  **历史管理增强**
  - 历史页支持关键词搜索（占问、本卦、变卦）与多条件筛选（全部/仅收藏、起卦方式组合）。
  - 历史页与结果页均可直接收藏/取消收藏，收藏状态持久保存。

-  **结果与会话分享**
  - 结果页支持一键导出 Markdown（含基本排盘信息，若有 AI 结论会附带）。
  - AI 会话支持复制最后助手回复、重试上一问、导出会话 Markdown。
...
- **恢复支持冲突策略与统计**：可选择重复记录“跳过/覆盖”，并反馈新增/覆盖/跳过数量。
- **Prompt 自定义受保护**：自定义提示词会被标记为用户配置，不会被版本升级自动覆盖。

### 历史管理与导入策略（v1.1）

- 历史筛选支持组合条件：`仅收藏 + 起卦方式多选 + 关键词`。
- 恢复导入默认进入“预览确认”流程：先看记录清单，再执行导入。
- 若备份中 `settings.apiKey` 为空，恢复时会保留当前本机已有密钥，避免误清空。
